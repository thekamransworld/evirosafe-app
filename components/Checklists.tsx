                    ...
                    {checklistTemplates.map(template => (
                        <li key={template.id} className="py-3">
                           <div className="flex justify-between items-center">
                               <div>
                                   <p className="text-sm font-medium text-gray-900">{getTranslated(template.title)}</p>
            ...
            <Card title="Recent Checklist Runs">
                ...
                <table className="min-w-full divide-y divide-gray-200">
                    ...
                    {checklistRunList.map((run) => (
                        <tr key={run.id}>
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{getTemplateTitle(run.template_id)}</td>
      ...
      {isSetupModalOpen && selectedTemplate && (
         <div className="fixed inset-0 bg-black bg-opacity-50 z-40 flex justify-center items-center" onClick={() => setSetupModalOpen(false)}>
            <div className="bg-white rounded-lg shadow-xl w-full max-w-md" onClick={e => e.stopPropagation()}>
                <div className="p-6">
                    <h3 className="text-lg font-bold">Run Checklist</h3>
                    <p className="text-sm text-gray-600 mt-1">Setup the details for this checklist run.</p>
                    <div className="mt-4 space-y-4">
                        <div>
                            <label className="block text-sm font-medium text-gray-700">Checklist Template</label>
                            <input type="text" readOnly value={getTranslated(selectedTemplate.title)} className="mt-1 w-full p-2 bg-gray-100 border border-gray-300 rounded-md" />
                        </div>
                        <div>
                             <label htmlFor="project" className="block text-sm font-medium text-gray-700">Select Project</label>
                             <select id="project" value={projectForRun?.id} onChange={e => setProjectForRun(projects.find(p => p.id === e.target.value) || null)} className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm rounded-md">
