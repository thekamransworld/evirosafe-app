    ...
    <Card className="flex flex-col hover:shadow-lg transition-shadow duration-300">
      <div className="flex-grow">
        <div className="flex justify-between items-start">
          <span className="text-xs font-semibold uppercase tracking-wider text-primary-700 bg-primary-100 px-2 py-1 rounded-full">{plan.type}</span>
          <Badge color={getStatusColor(plan.status)}>{plan.status.replace('_', ' ')}</Badge>
        </div>
        <h3 className="text-lg font-bold text-text-primary mt-3">{plan.title}</h3>
        <p className="text-sm text-text-secondary">{plan.version}</p>
      ...
      <div className="border-t mt-4 pt-4 text-xs text-gray-500 space-y-2">
        <p><strong>Prepared By:</strong> {plan.people.prepared_by.name}</p>
        <p><strong>Next Review:</strong> {new Date(plan.dates.next_review_at).toLocaleDateString()}</p>
      </div>
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold text-text-primary">HSE Plans Library</h1>
        {can('create', 'plans') && (
      ...
      <Card className="mb-6">
          <div className="space-y-4">
              ...
              <div>
                  <label className="text-sm font-semibold text-gray-600">Plan Type</label>
                  <div className="flex flex-wrap gap-2 mt-2">
