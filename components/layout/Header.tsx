...
export const Header: React.FC<HeaderProps> = ({ activeOrg, setActiveOrg, organizations, user, toggleSidebar }) => {
  ...
  const { language, handleUpdateUser, logout, theme, toggleTheme } = useAppContext();
          ...
          {userDropdownOpen && (
            <div className="absolute right-0 mt-2 w-56 bg-white dark:bg-dark-card rounded-lg shadow-lg border border-border-color dark:border-dark-border z-50">
                <div className="p-1">
                    ...
                    <button onClick={logout} className="block w-full text-left px-3 py-2 text-sm text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-md">Sign Out</button>
                </div>
            </div>
          )}
        </div>
