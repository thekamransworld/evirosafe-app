...
export async function exportReportToPdf(
  ...
  // exportTableToPdf's table content paginates itself via autoTable, but this
  // function positions text manually — nothing here previously stopped a long
  // description or a long CAPA list from running off the bottom of the page.
  // Call this before writing anything that might not fit in the space left.
  ...
  // Description — draw line by line (not as one block) so a long description
  // naturally continues onto a new page instead of running off the bottom.
  // jsPDF doesn't auto-paginate a multi-line doc.text() call; lines placed
  // past the page height are simply never rendered, which is worse than an
  // overlap — the text silently disappears from the output PDF.
  ensureSpace(9);
  ...
  if (report.capa?.length) {
    ...
    (doc as any).autoTable({
      ...
      // autoTable paginates its own rows automatically once startY is valid —
      // that part already worked, it just needed a safe startY to begin from.
    });
  }

  // Footer — draw on every page now that the report can span more than one
  // (previously only appeared on the last page, since doc.text() only
  // touches the page the cursor is currently on).
  const totalPages = doc.internal.getNumberOfPages();
  for (let p = 1; p <= totalPages; p++) {
    doc.setPage(p);
    doc.setFontSize(8); doc.setTextColor(150, 150, 150);
    doc.text(
      `Generated by ${orgName} on ${new Date().toLocaleString('en-GB')} · CONFIDENTIAL`,
      margin, pageH - 8,
    );
