...
export const DocumentControl: React.FC = () => {
  ...
  const handleCreate = (data: Omit<ControlledDocument, 'id' | 'org_id'>) => {
    handleCreateControlledDocument({ ...data, id: `doc_${Date.now()}` } as ControlledDocument);
  };
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <CanDo permission="document:create">
          <Button leftIcon={<Plus className="w-4 h-4" />} onClick={() => setShowCreate(true)}>New Document</Button>
        </CanDo>
      </div>
      ...
      <DocumentCreationModal isOpen={showCreate} onClose={() => setShowCreate(false)} onSubmit={handleCreate} ownerName={activeUser?.name ?? 'Unknown'} />
    </div>
  );
};