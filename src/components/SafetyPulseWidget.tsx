...
const KPICard: React.FC<{ title: string; value: string; change: number; color: string }> = ({ title, value, change, color }) => (
    <div className="metric-card flex flex-col justify-between p-3">
        <div>
...
export const SafetyPulseWidget: React.FC<SafetyPulseWidgetProps> = ({ onExpand }) => {
    const [data, setData] = useState(() => generateTrendData(24)); // Last 2 hours roughly
    ...
    return (
        <div className="safety-pulse-card h-full flex flex-col overflow-hidden">
            {/* Header */}
