...
// --- MANPOWER & MANHOURS INPUT COMPONENT ---
const ManpowerStatistics: React.FC = () => {
    ...
    const [inputs, setInputs] = useState({
        clientManpower: 5,
        consultantManpower: 8,
        contractorManpower: 50,
        subcontractorManpower: 135,
        clientHours: 800,
        consultantHours: 1280,
        contractorHours: 10000,
        subcontractorHours: 27000,
        overtimeHours: 450
    });

    // Auto-calculate totals
    const totalManpower = inputs.clientManpower + inputs.consultantManpower + inputs.contractorManpower + inputs.subcontractorManpower;
    const totalManhours = inputs.clientHours + inputs.consultantHours + inputs.contractorHours + inputs.subcontractorHours;

    // Chart Data (Dynamic based on inputs for the selected month)
    // In a real app, you would fetch historical data for other months.
    // Here we simulate history and update the *last* entry based on selection.
    const chartData = [
        { name: 'Jan', manhours: 35000, incidents: 2 },
        { name: 'Feb', manhours: 38000, incidents: 1 },
        { name: 'Mar', manhours: 32000, incidents: 4 },
        { name: 'Apr', manhours: 36000, incidents: 1 },
        { name: 'May', manhours: 39000, incidents: 2 },
        { name: selectedMonth, manhours: totalManhours, incidents: 0 }, // Dynamic based on selection
    ];
    ...
    return (
        <div className="glass-card p-6 mt-6">
            <div className="flex justify-between items-center mb-6 border-b border-slate-700 pb-2">
                <h3 className="text-lg font-bold text-white">HSE Statistics & Manhours</h3>
                <div className="flex items-center gap-2">
                    <span className="text-xs text-slate-400 uppercase font-bold">Select Month:</span>
                    <select 
                        value={selectedMonth} 
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                {/* LEFT: CHART (Visual Representation) */}
                <div className="h-80 w-full bg-slate-900/50 rounded-xl border border-slate-700 p-4">
                    <h4 className="text-xs font-bold text-slate-400 uppercase mb-4">Manhours vs Incidents Trend</h4>
                    <ResponsiveContainer width="100%" height="100%">
                        <ComposedChart data={chartData}>
                            ...
                            />
                            <Legend />
                            <Bar yAxisId="left" dataKey="manhours" name="Manhours" fill="#3b82f6" barSize={20} radius={[4, 4, 0, 0]} />
                            <Line yAxisId="right" type="monotone" dataKey="incidents" name="Incidents" stroke="#f87171" strokeWidth={3} dot={{r: 4}} />
                        </ComposedChart>
                    </ResponsiveContainer>
                </div>

                {/* RIGHT: DATA ENTRY TABLE (User Input) */}
                    ...
                    <table className="w-full text-sm text-left text-slate-300">
                        <thead className="text-xs text-slate-400 uppercase bg-slate-800">
                            <tr>
                                ...
                                <th className="px-4 py-3 text-center bg-slate-700/50 border-x border-slate-600">Current ({selectedMonth})</th>
                                <th className="px-4 py-3 text-center">Target</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-700 bg-slate-900/50">
                            {/* Manpower Section */}
                            <InputRow label="Manpower - Client" prev={5} value={inputs.clientManpower} onChange={(v) => handleInputChange('clientManpower', v)} target={6} />
                            <InputRow label="Manpower - Consultant" prev={8} value={inputs.consultantManpower} onChange={(v) => handleInputChange('consultantManpower', v)} target={10} />
                            <InputRow label="Manpower - Contractor" prev={45} value={inputs.contractorManpower} onChange={(v) => handleInputChange('contractorManpower', v)} target={55} />
                            <InputRow label="Manpower - Subcontractor" prev={120} value={inputs.subcontractorManpower} onChange={(v) => handleInputChange('subcontractorManpower', v)} target={140} />
                            
                            {/* Divider Row */}
                            <tr className="bg-slate-800/50"><td colSpan={4} className="py-1"></td></tr>

                            {/* Manhours Section */}
                            <InputRow label="Manhours - Client" prev={800} value={inputs.clientHours} onChange={(v) => handleInputChange('clientHours', v)} target={1000} unit="h" />
                            <InputRow label="Manhours - Consultant" prev={1280} value={inputs.consultantHours} onChange={(v) => handleInputChange('consultantHours', v)} target={1500} unit="h" />
                            <InputRow label="Manhours - Contractor" prev={9000} value={inputs.contractorHours} onChange={(v) => handleInputChange('contractorHours', v)} target={11000} unit="h" />
                            <InputRow label="Manhours - Subcontractor" prev={24000} value={inputs.subcontractorHours} onChange={(v) => handleInputChange('subcontractorHours', v)} target={28000} unit="h" />
                            <InputRow label="Overtime Hours" prev={500} value={inputs.overtimeHours} onChange={(v) => handleInputChange('overtimeHours', v)} target={400} unit="h" />

                            {/* Totals Row */}
                            <tr className="bg-slate-800 font-bold text-white">
                                <td className="px-4 py-3">TOTAL MANHOURS</td>
                                <td className="px-4 py-3 text-center">35,580</td>
                                <td className="px-4 py-3 text-center text-emerald-400 text-lg border-x border-slate-600">
                                    {totalManhours.toLocaleString()}
                                </td>
                                <td className="px-4 py-3 text-center">41,900</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
...
const InputRow: React.FC<{ label: string, prev: number, value: number, onChange: (val: string) => void, target: number, unit?: string }> = ({ label, prev, value, onChange, target, unit }) => (
    <tr className="hover:bg-slate-800/50 transition-colors">
        <td className="px-4 py-2 font-medium">{label}</td>
        <td className="px-4 py-2 text-center text-slate-500">{prev}</td>
        <td className="px-4 py-1 text-center border-x border-slate-700 bg-slate-800/30">
            <input 
                type="number" 
                value={value} 
                onChange={(e) => onChange(e.target.value)}
                className="w-20 bg-slate-700 border border-slate-600 rounded px-2 py-1 text-center text-white focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
            />
        </td>
        <td className="px-4 py-2 text-center text-slate-500">{target} {unit}</td>
    </tr>
);
