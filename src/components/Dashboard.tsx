...
const AnimatedMetric: React.FC<{ value: string | number; className?: string }> = ({ value, className }) => {
    ...
    useEffect(() => {
        if (prevValue.current !== value) {
            setAnimating(true);
            const timer = setTimeout(() => setAnimating(false), 300); // Matches 'bump' animation duration
...
export const Dashboard: React.FC = () => {
    ...
    // Simulate live data changes for micro-movements
    useEffect(() => {
        const interval = setInterval(() => {
            // Randomly fluctuate workforce (+/- 1 or 0)
            setSimulatedWorkforce(prev => Math.max(10, prev + (Math.random() > 0.5 ? 1 : -1) * (Math.random() > 0.7 ? 1 : 0)));
            // Randomly fluctuate temp (+/- 0.5 or 0)
    ...
    return (
        <div className="dashboard-bg">
            ...
            {/* ROW 3: SAFETY PULSE */}
            <div className="h-80">
                 {/* Using the SafetyPulseWidget but wrapped in our layout container style handled by itself mostly, 
                     but here we ensure it fits the grid. The widget itself has internal card styling. */}
