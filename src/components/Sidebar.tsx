...
import type { View, Resource } from '../types'; // <--- FIXED: uses ..
import { logoSrc } from '../config';            // <--- FIXED: uses ..
import { useAppContext } from '../contexts';    // <--- FIXED: uses ..
import { useAuth } from '../contexts/AuthContext'; // <--- FIXED: uses ..
...
export const Sidebar: React.FC<SidebarProps> = ({ currentView, setCurrentView, isOpen, setOpen }) => {
  ...
  return (
    <div className={`shrink-0 glass-panel h-screen flex flex-col transition-all duration-300 z-50 ${isOpen ? 'w-64' : 'w-20'}`}>
        ...
        {/* User Footer - NOW USING FIREBASE USER */}
...
const MapIcon = (props: React.SVGProps<SVGSVGElement>) => (<svg {...props} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M9 6.75V15m6-6v8.25m.503 3.498l4.875-2.437c.381-.19.622-.58.622-1.006V4.82c0-.836-.88-1.38-1.628-1.006l-3.869 1.934c-.317.159-.69.159-1.006 0L9.503 3.252a1.125 1.125 0 00-1.006 0L3.622 5.689C3.24 5.88 3 6.27 3 6.695V19.18c0 .836.88 1.38 1.628 1.006l3.869-1.934c.317-.159.69-.159 1.006 0l4.994 2.497c.317.158.69.158 1.006 0z" /></svg>);
