...
const PlanCard: React.FC<{ plan: Plan; onSelect: (plan: Plan) => void }> = ({ plan, onSelect }) => {
  // Safety check: Default to 'draft' if status is missing
    ...
    <Card className="flex flex-col hover:shadow-lg transition-shadow duration-300">
      <div className="flex-grow">
        <div className="flex justify-between items-start">
          <span className="text-xs font-semibold uppercase tracking-wider text-primary-700 bg-primary-100 dark:bg-primary-900/30 dark:text-primary-300 px-2 py-1 rounded-full">
            {plan.type}
          </span>
          <Badge color={getStatusColor(safeStatus)}>
      ...
      <div className="border-t dark:border-white/10 mt-4 pt-4 text-xs text-gray-500 dark:text-gray-400 space-y-2">
        <p><strong>Prepared By:</strong> {plan.people?.prepared_by?.name || 'Unknown'}</p>
        <p><strong>Next Review:</strong> {plan.dates?.next_review_at ? new Date(plan.dates.next_review_at).toLocaleDateString() : 'Not set'}</p>
      </div>
...
export const Plans: React.FC<PlansProps> = ({ onSelectPlan, onNewPlan }) => {
  ...
  const filteredPlans = useMemo(() => {
    return planList.filter(plan => {
      ...
      const statusMatch = statusFilter === 'All' || safeStatus === statusFilter;
          ...
          <div className="space-y-4">
              ...
              <div>
                  ...
                  <div className="flex flex-wrap gap-2 mt-2">
                      ...
                      {['draft', 'under_review', 'approved', 'published', 'archived'].map(s => <FilterButton key={s} label={s.replace('_', ' ')} value={s} currentFilter={statusFilter} setFilter={setStatusFilter} />)}
                  </div>
              </div>
          </div>
      </Card>
