...
// Fixed: was using 13/7 thresholds. Now uses 15/8/4 (ISO 31000)
const getRiskColorClass = (severity: number, likelihood: number): string => {
    const score = severity * likelihood;
    if (score >= 15) return 'bg-red-200 hover:bg-red-300';
    if (score >= 8)  return 'bg-orange-200 hover:bg-orange-300';
    if (score >= 4)  return 'bg-yellow-200 hover:bg-yellow-300';
    return 'bg-green-200 hover:bg-green-300';
};

// Maps risk color to a CSS color string for inline style use
const riskColorToHex: Record<string, string> = {
  red:    '#ef4444',
  orange: '#f97316',
  yellow: '#ca8a04',
  green:  '#16a34a',
...
export const RiskMatrixInput: React.FC<RiskMatrixInputProps> = ({ value, onChange }) => {
  // Fixed: was calling getRiskLevel which returned an object; now uses getRiskResult correctly
  const currentRisk = getRiskResult(value);
        ...
        <div className="flex flex-col md:flex-row gap-4">
            ...
            <div className="p-4 bg-gray-50 rounded-lg flex-grow">
                ...
                <div
                  ...
                  style={{ color: riskColorToHex[currentRisk.color] ?? '#16a34a' }}
                >
                    {currentRisk.level}
                </div>
                <div className="mt-1 text-sm font-semibold" style={{ color: riskColorToHex[currentRisk.color] ?? '#16a34a' }}>
                ...
                <p className="mt-3 text-xs text-gray-500 italic border-t pt-2">{currentRisk.action}</p>
            </div>
        </div>
    </div>
  );
