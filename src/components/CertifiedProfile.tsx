...
function generateCertificateId(level: CertificateLevel, countryCode: string) {
  ...
  const levelCode = safeLevel.charAt(0);
      ...
      {activeTab === 'requirements' && (
        <Card title="Global Certification Rules (EviroSafe International)">
          <div className="space-y-8 text-gray-200">
            ...
            <section>
              <h3 className="text-lg font-bold">4) Application Workflow</h3>
              <div className="mt-3 grid grid-cols-1 md:grid-cols-4 gap-3 text-sm">
                {['Draft', 'Submitted', 'Under Review', 'Approved'].map((s) => (
                  <div key={s} className="rounded-xl border border-gray-700 bg-black/20 p-4">
                    <div className="font-bold text-gray-100">{s}</div>
                    <div className="text-gray-400 mt-1">Controlled status with evidence and audit trail.</div>
                  </div>
                ))}
              </div>
            </section>
